<template>
    ...
    <div class="bg-white min-h-[85vh]">
        <a-button :disabled="usersToRemove.length === 0"
                  class="m-5"
                  danger
                  @click="confirmRemoveUser">
            Loại bỏ
        </a-button>
        <a-tree :selectable="false"
                ...
                <template v-else-if="parNotHaveSubMenu.includes(dataRef.key)">
                    <div class="space-x-1.5">
                        ...
                        <span v-if="dataRef.children && dataRef.children.length"
                              class="cursor-default font-normal">
...
onMounted(() => {
    const decodedInfoUser = decodeJWT(localStorage.getItem(ACCESS_TOKEN_KEY));
